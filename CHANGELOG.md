...
### CLI
- Added `lattice init` command to create `.lattice/config.json` configuration file
- Supports required flags: `--projectType` (nextjs|expo-eas) and `--preset` (startup|pro|enterprise)
- Supports optional flags: `--billing`, `--analytics`, `--observability`, `--testing`
- Prevents overwriting existing config unless `--force` is provided
- `lattice generate` now automatically uses `.lattice/config.json` if it exists

### Harness
- Integrated `lattice init` into fixture validation pipeline
- Harness now calls `init` before `generate` in each fixture
- Ensures fixtures have proper config files generated by init
